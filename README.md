...
1. **vsftpd версии 2.3.4**  
   Данная версия FTP-сервера содержит встроенный backdoor, позволяющий получить удалённый доступ к системе при специально сформированном имени пользователя.  
   Ссылка: https://www.exploit-db.com/exploits/17491

2. **Samba версии 3.0.20**  
   Уязвимость в механизме обработки имён пользователей позволяет выполнить произвольные команды на сервере с повышенными привилегиями.  
   Ссылка: https://www.exploit-db.com/exploits/16320

3. **Служба distccd**  
   Сервис предназначен для распределённой компиляции, однако в данной конфигурации допускает выполнение команд без какой-либо аутентификации.  
